 ...
 * @return array
 */
function travel_base_body_classes( $classes ) {
	...
	if ( is_front_page() ) {
		...
		$slider_enable = ( false === $options[ 'slider_section_enable'] ) ? 'featured-slider-disabled' : '';
		$classes[] = esc_attr( $slider_enable );
	}
	...
	if ( is_singular() || is_home() ) {
		$id = ( is_home() && ! is_front_page() ) ? get_option( 'page_for_posts' ) : get_the_id();
	  	$sidebar = get_post_meta( $id, 'travel-base-selected-sidebar', true );
...
add_filter( 'body_class', 'travel_base_body_classes' );
