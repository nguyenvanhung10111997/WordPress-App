...
function travel_base_search_form() { ?>
	<div class="wp-travel-search">
		<form method="get" name="wp-travel_search" action="<?php echo esc_url( home_url( '/' ) );  ?>" > 
			...
			<p>
				<?php $placeholder = esc_attr__( 'Ex: Trekking', 'travel-base' ); ?>
			...
			<p>
				...
				$args = array(
					'show_option_all'    => esc_attr__( 'Trip Type', 'travel-base' ),
					'hide_empty'         => 0,
					'selected'           => 1,
					'hierarchical'       => 1,
					'name'               => $taxonomy,
			...
			<p>
				...
				$args = array(
					'show_option_all'    => esc_attr__( 'Location', 'travel-base' ),
					'hide_empty'         => 0,
					'selected'           => 1,
					'hierarchical'       => 1,
					'name'               => $taxonomy,
			...
			<p class="wp-travel-search"><input type="submit" name="wp-travel_search" id="wp-travel-search" class="button button-primary" value="<?php esc_attr_e( 'Search', 'travel-base' ) ?>"  /></p>
		</form>
	</div>	
	<?php
}
add_filter( 'wp_travel_search_form', 'travel_base_search_form', 999 );
 ...
 */
function travel_base_trip_duration( $post_id ) {
	if ( ! $post_id ) {
		return;
	}
	...
	if ( 'yes' === $fixed_departure ) : 
		...
		if ( $trip_duration || $trip_duration_night ) :
			printf( __( '%1$s Day(s) %2$s Night(s)', 'travel-base' ), $trip_duration, $trip_duration_night );
		endif;
...
add_action( 'wp_travel_after_main_content', 'travel_base_archive_wrapper_close' );
/**
 * Add html for Keywords.
 */
function travel_base_archive_wrapper_close() { ?>
	</div>
<?php }